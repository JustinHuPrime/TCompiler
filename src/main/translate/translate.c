  ...
  switch (fromType->kind) {
    ...
    case K_CHAR: {
      switch (toType->kind) {
        ...
        case K_SHORT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_ZX_SHORT,
                       TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
          return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
        }
        case K_UINT:
        case K_INT:
        case K_WCHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_ZX_INT,
                       TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
          return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
        }
        case K_LONG:
        case K_ULONG: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_ZX_LONG,
                       TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
          return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
        }
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_U_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_U_TO_DOUBLE,
        ...
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out,
             BINOP(BYTE_WIDTH, IO_NE, TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP),
                   from, UBYTE(0)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          size_t temp = NEW(tempAllocator);
          size_t size = typeSizeof(toType);
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(BYTE_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
          ...
          if (size == BYTE_WIDTH) {
            return from;
          } else if (size == SHORT_WIDTH) {
            size_t temp = NEW(tempAllocator);
            IR(out, UNOP(BYTE_WIDTH, IO_ZX_SHORT,
                         TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
            return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
          } else if (size == INT_WIDTH) {
            size_t temp = NEW(tempAllocator);
            IR(out, UNOP(BYTE_WIDTH, IO_ZX_INT,
                         TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
            return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
          } else if (size == LONG_WIDTH) {
            size_t temp = NEW(tempAllocator);
            IR(out, UNOP(BYTE_WIDTH, IO_ZX_LONG,
                         TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
            return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
          } else {
            error(__FILE__, __LINE__, "invalid enum size");
          }
        }
        default: { error(__FILE__, __LINE__, "invalid type being cast to"); }
      }
    }
    case K_BYTE: {
      switch (toType->kind) {
        ...
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_S_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(BYTE_WIDTH, IO_S_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
    ...
    case K_USHORT: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out,
             MOVE(BYTE_WIDTH, TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
        ...
        case K_WCHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_ZX_INT,
                       TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
          return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
        }
        case K_LONG:
        case K_ULONG: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_ZX_LONG,
                       TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
          return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
        }
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_U_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_U_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out,
             BINOP(SHORT_WIDTH, IO_NE,
                   TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from, UBYTE(0)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          size_t temp = NEW(tempAllocator);
          ...
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(SHORT_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
    ...
    case K_SHORT: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_TRUNC_BYTE,
                       TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
        ...
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_S_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(SHORT_WIDTH, IO_S_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
    ...
    case K_WCHAR: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_TRUNC_BYTE,
                       TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_TRUNC_SHORT,
                       TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
          return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
        }
        case K_UINT:
        ...
        case K_ULONG: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_ZX_LONG,
                       TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
          return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
        }
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_U_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_U_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out,
             BINOP(INT_WIDTH, IO_NE, TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP),
                   from, UBYTE(0)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          ...
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(INT_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
    ...
    case K_INT: {
      switch (toType->kind) {
        ...
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_S_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(INT_WIDTH, IO_S_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
    ...
    case K_ULONG: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_TRUNC_BYTE,
                       TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_TRUNC_SHORT,
                       TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
          return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
        }
        case K_UINT:
        ...
        case K_WCHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_TRUNC_INT,
                       TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
          return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
        }
        case K_LONG:
        ...
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_U_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_U_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out,
             BINOP(LONG_WIDTH, IO_NE, TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP),
                   from, UBYTE(0)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          ...
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(LONG_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
    ...
    case K_LONG: {
      switch (toType->kind) {
        ...
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_S_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(LONG_WIDTH, IO_S_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
    ...
    case K_FLOAT: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(FLOAT_WIDTH, IO_F_TO_BYTE,
                       TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(FLOAT_WIDTH, IO_F_TO_SHORT,
                       TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
          return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
        }
        case K_UINT:
        ...
        case K_WCHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(FLOAT_WIDTH, IO_F_TO_INT,
                       TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
          return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
        }
        case K_LONG:
        case K_ULONG: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(FLOAT_WIDTH, IO_F_TO_LONG,
                       TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
          return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
        }
        case K_FLOAT: {
        ...
        case K_DOUBLE: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(FLOAT_WIDTH, IO_F_TO_DOUBLE,
                       TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE), from));
          return TEMP(temp, DOUBLE_WIDTH, DOUBLE_WIDTH, AH_SSE);
        }
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out, BINOP(FLOAT_WIDTH, IO_FP_NE,
                        TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from,
                        FLOAT(FLOAT_BITS_ZERO)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          size_t temp = NEW(tempAllocator);
          ...
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(FLOAT_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
    ...
    case K_DOUBLE: {
      switch (toType->kind) {
        ...
        case K_CHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(DOUBLE_WIDTH, IO_F_TO_BYTE,
                       TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_USHORT:
        case K_SHORT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(DOUBLE_WIDTH, IO_F_TO_SHORT,
                       TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP), from));
          return TEMP(temp, SHORT_WIDTH, SHORT_WIDTH, AH_GP);
        }
        case K_UINT:
        ...
        case K_WCHAR: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(DOUBLE_WIDTH, IO_F_TO_INT,
                       TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP), from));
          return TEMP(temp, INT_WIDTH, INT_WIDTH, AH_GP);
        }
        case K_LONG:
        case K_ULONG: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(DOUBLE_WIDTH, IO_F_TO_LONG,
                       TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP), from));
          return TEMP(temp, LONG_WIDTH, LONG_WIDTH, AH_GP);
        }
        case K_FLOAT: {
          size_t temp = NEW(tempAllocator);
          IR(out, UNOP(DOUBLE_WIDTH, IO_F_TO_FLOAT,
                       TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE), from));
          return TEMP(temp, FLOAT_WIDTH, FLOAT_WIDTH, AH_SSE);
        }
        case K_DOUBLE: {
        ...
        case K_BOOL: {
          size_t temp = NEW(tempAllocator);
          IR(out, BINOP(DOUBLE_WIDTH, IO_FP_NE,
                        TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP), from,
                        DOUBLE(DOUBLE_BITS_ZERO)));
          return TEMP(temp, BYTE_WIDTH, BYTE_WIDTH, AH_GP);
        }
        case K_UNION: {
          size_t temp = NEW(tempAllocator);
          ...
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(DOUBLE_WIDTH, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        case K_ENUM: {
    ...
    case K_STRUCT: {
      switch (toType->kind) {
        ...
        case K_UNION: {
          size_t temp = NEW(tempAllocator);
          size_t fromSize = typeSizeof(fromType);
          size_t size = typeSizeof(toType);
          size_t alignment = typeAlignof(toType);
          AllocHint kind = typeKindof(toType);
          IR(out, OFFSET_STORE(fromSize, TEMP(temp, size, alignment, kind),
                               from, ULONG(0)));
          return TEMP(temp, size, alignment, kind);
        }
        default: { error(__FILE__, __LINE__, "invalid type being cast to"); }
    ...
    default: { error(__FILE__, __LINE__, "invalid type being cast"); }
  }
}
